...
|------|----------|-------------|-------|
| **npm 包** | `opencode-ai` | `@anthropic-ai/claude-code` | `@openai/codex` |
| **二进制** | `opencode` | `claude` | `codex` |
| **厂商** | 独立开源 | Anthropic 官方 | OpenAI 官方 |
...
> 三者的技能 `SKILL.md` 格式兼容（都是 name/description frontmatter），技能内容完全一致，只是目录位置不同。
