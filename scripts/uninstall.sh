...
Options:
  ...
  --remove-packages     同时卸载全局 npm 包 opencode-ai、@jackwener/opencli
...
echo ""
printf '%b即将清理以下内容：%b\n' "$YELLOW" "$NC"
printf '  1. OpenCode 配置：%s（会先备份）\n' "$CONFIG_FILE"
[[ -n "$VAULT" && -d "$VAULT" ]] && printf '  2. vault 插件配置：%s/.obsidian/plugins/opencode-obsidian/\n' "$VAULT"
[[ "$REMOVE_VAULT" -eq 1 ]] && printf '%b3. 整个 vault 目录：%s（含所有笔记！）%b\n' "$RED" "$VAULT" "$NC"
[[ "$REMOVE_PACKAGES" -eq 1 ]] && printf '  4. npm 全局包：opencode-ai、@jackwener/opencli\n'
echo ""
...
# 1. 删除 OpenCode 配置（先备份）
if [[ -f "$CONFIG_FILE" ]]; then
  BACKUP="$CONFIG_FILE.uninstall-$(date +%Y%m%d-%H%M%S)"
  cp "$CONFIG_FILE" "$BACKUP"
  rm -f "$CONFIG_FILE"
  printf '%b✓ 已删除 OpenCode 配置，备份在：%s%b\n' "$GREEN" "$BACKUP" "$NC"
else
  printf '%b· OpenCode 配置不存在，跳过%b\n' "$YELLOW" "$NC"
fi

# 2. 删除插件配置（不删 vault 本身）
if [[ -n "$VAULT" && -d "$VAULT/.obsidian/plugins/opencode-obsidian" ]]; then
  rm -rf "$VAULT/.obsidian/plugins/opencode-obsidian"
  printf '%b✓ 已删除 Obsidian 插件配置%b\n' "$GREEN" "$NC"
else
...
if [[ "$REMOVE_PACKAGES" -eq 1 ]]; then
  for pkg in opencode-ai @jackwener/opencli; do
